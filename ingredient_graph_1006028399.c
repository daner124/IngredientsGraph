...
void related_ingredients(char source_ingredient[MAX_STR_LEN])
{
    ...
    * ********/
	...
	int col = ingredient_index(source_ingredient);
    ...
    *******/
	...
	else
	{
		...
		for(int row =0; row < MAT_SIZE; row++)
		{
			if(AdjMat[col][row] > 0)
			{
				h = related_k_dist(h, ingredients[row], k, dist);
			}
		}
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
    ...
    *****/
	...
	intNode *headAvoid = NULL;
	intNode *headSource = NULL;
	intNode *p = NULL;
	intNode *q = NULL;
	
	//gets items avoiding 
	headAvoid = related_k_dist(headAvoid, avoid, k_avoid, 0);
	
	//adds in item itself to avoid
	int startAvoid = 0;
	startAvoid += ingredient_index(avoid);
	
	if(searchInt(headAvoid, startAvoid) == 0)
	{
		headAvoid = insertInt(headAvoid, startAvoid);
	}
	
	//adds items in range with avoiding at end
	headSource = related_k_dist(headAvoid, source_ingredient, k_source, 0);
...
//Adds rows to find max ingredient
int sum_calc(intNode *h, char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN], int max_val, int max_ind) 
{
	//printf("\n%d %d\n", max_val, max_ind);
	if (h == NULL) 
		return max_ind;
	...
	while (strcmp(recipe[i], "") != 0)
	{
		if(strcmp(recipe[i], to_change) != 0)
		{
			sum += AdjMat[h->x][ingredient_index(recipe[i])];
		}
	...
	{
		max_val = sum;
		max_ind = ingredient_index(ingredients[h->x]);
	}
	
	return sum_calc(h->next, recipe, to_change, max_val, max_ind);
}
   ...
   */
    ...
    ******/
	...
	while (strcmp(recipe[i], "") != 0)
	{
		if(strcmp(recipe[i], to_change) != 0)
		{
			//Gets related avoiding to_change
			new_h = related_with_restrictions(recipe[i], to_change, 0, 0);
			
			//Goes to end of new and connects to whole list
			p = new_h;
			while (p != NULL)
			{
				if (searchInt(h, p->x) == 0)
					h = insertInt(h, p->x);
				p = p->next;
			}
			//deletes temporary list
			new_h = deleteList(new_h);
		}
	...
	*/
	...
	ind_new += sum_calc(h, recipe, to_change, 0, 0);
	
	//printf("\n\n\n%d %s\n", ind_new, ingredients[ind_new]);
	...
	for (int j = 0; j < 9; j++)
	{
		...
		{
			strcpy(recipe[j], ingredients[ind_new]);
			break;
		}
	}
